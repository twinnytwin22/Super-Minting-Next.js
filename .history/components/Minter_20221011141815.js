...
const Minter = () => {
    ...
    const { data, isLoading, isSuccess, publicPurchase } = useContractWrite(config)

function handleMint = async () => {
            try {
                setMintLoading(true);
                const response = await publicPurchase(address.toString(), BigNumber.from(mintAmount), {
